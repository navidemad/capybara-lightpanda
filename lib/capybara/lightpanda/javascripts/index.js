  ...
  var XPathEval = (function() {
    ...
    function getAxisNodes(node, axis) {
      ...
      switch (axis) {
        ...
        case 'ancestor':
          // Reverse axis — XPath returns node-sets in document order, so emit outermost first.
          p = node.parentNode; while (p) { out.unshift(p); p = p.parentNode; } break;
        case 'ancestor-or-self':
          p = node.parentNode; while (p) { out.unshift(p); p = p.parentNode; } out.push(node); break;
        case 'following-sibling':
          c = node.nextSibling; while (c) { out.push(c); c = c.nextSibling; } break;
        case 'preceding-sibling':
          // Reverse axis — emit earliest first.
          c = node.previousSibling; while (c) { out.unshift(c); c = c.previousSibling; } break;
        case 'following':
