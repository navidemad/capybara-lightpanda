...
// HTMLElement.draggable IDL semantics. Lightpanda doesn't implement the
// property (probed 2026-08-24 on nightly 8781: `el.draggable` is undefined
// even with draggable="true"; only the attribute *name* is interned
// upstream), so Node#drag_to's scripts resolve draggability through this
// helper. Native-first: the `typeof` guard hands back the browser's own
// answer the day upstream ships the getter. Spec (WHATWG "draggable"):
// attribute "true"/"false" wins; defaults are <img> and <a href>.
function isDraggable(el) {
  if (typeof el.draggable === 'boolean') return el.draggable;
  var attr = el.getAttribute && el.getAttribute('draggable');
  if (attr === 'true') return true;
  if (attr === 'false') return false;
  var tag = (el.tagName || '').toUpperCase();
  return tag === 'IMG' || (tag === 'A' && !!(el.hasAttribute && el.hasAttribute('href')));
}
