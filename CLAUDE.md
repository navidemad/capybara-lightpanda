...
bundle exec rake test:js              # Run the Bun harness for the injected _lightpanda predicate logic
                                      #   (test/js/*.test.ts, ~0.1s). Dev-only; skips with a notice if Bun is absent.
...
- `Browser` is composed of include-modules under `lib/capybara/lightpanda/browser/` — `Runtime` (JS evaluation/serialization), `Finder` (element finding), `Navigation` (go_to/back/forward/refresh + readyState fallback), `Modals`, `Console` (consoleAPICalled consumers incl. Turbo sentinels). `browser.rb` keeps lifecycle/reset/reconnect, the at_exit registry, command plumbing, page-state readers, frames, and `wait_for_idle`. Instance variables are initialized in `Browser#initialize` regardless of which module uses them.
...
- The injected `_lightpanda` bundle lives in `javascripts/` split by concern: `turbo.js` (Turbo activity tracking) and `predicates.js` (DOM visibility/state predicates — `_lightpanda.isVisible`, `isObscured`, `isDisabled`, `isContentEditable`, `visibleText`), with `attach.js` wiring them onto `window._lightpanda` and `banner.js` as a provenance header. None of these files use IIFE/`export`/`import` — `auto_scripts.rb` is the linker: it concatenates them (order: banner→turbo→predicates→attach) inside one IIFE and prepends the `if (window._lightpanda) return;` idempotency guard. Keeping the sources free of module syntax means each parses identically as a classic browser script and as a `new Function(...)` body, which is how the Bun harness (`test/js/`) tests the predicates without a build step. XPath is not polyfilled here — native `Document.evaluate` + `XPathResult` landed in Lightpanda PR #2305 (in nightly ≥6109) and the find paths (`browser/finder.rb`) call them directly. `Browser#create_page` registers the assembled `AutoScripts::JS` via `Page.addScriptToEvaluateOnNewDocument` so Lightpanda auto-injects it on every navigation. No manual re-injection needed.
- Node identity uses CDP remote object IDs (`Runtime.callFunctionOn` with `this` binding). All node operations route through a single `call` method for centralized error handling. JS function declarations may reference `_lightpanda.*` helpers — the bundle is registered via `Page.addScriptToEvaluateOnNewDocument`, so the namespace exists in every document (top frame and iframes alike).
...
- `Browser#set_viewport` (called from `create_page`) applies the `window_size` option via `Emulation.setDeviceMetricsOverride`. JS-visible viewport only — it moves `window.innerWidth`/`innerHeight` and what `matchMedia`/`@media` evaluate against; it is NOT layout, so nothing reflows and `getBoundingClientRect` stays synthetic. `Options::DEFAULT_WINDOW_SIZE` deliberately mirrors Lightpanda's native `Viewport.default` (1920×1080), not Cuprite's 1024×768, so the default is a no-op — do not "fix" it to match Cuprite without accepting that every existing suite's `@media` branches shift.
- `Browser#page_errors` is a **separate buffer from `#console_logs`** and must stay that way. Lightpanda emits no `Runtime.exceptionThrown`, so uncaught page exceptions are captured by passive `window` listeners in `javascripts/errors.js` reporting over the `console.debug` sentinel channel (`__lightpanda_page_error_` + JSON), which `Console#record_page_error` reroutes. Merging the two would diverge from Chrome (`exceptionThrown` ≠ `consoleAPICalled`) and from Playwright/Puppeteer (`pageerror` ≠ `console`), and would break suites asserting `console_logs` holds no errors. Every `consoleAPICalled` consumer filters on `driver_sentinel?`, not on one prefix. Retire the file when upstream ships the event — see wishlist B19 for the exact swap.
- `Node#hover` dispatches **both** `mouseover` (bubbling) and `mouseenter` (non-bubbling, per spec) via `HOVER_JS`. `mouseover` alone silently no-oped the `mouseenter->controller#open` Stimulus idiom — the dominant hover-menu pattern — and the failure surfaced far away as `ElementNotFound` on a menu item. CSS `:hover` still reveals nothing (no pointer state upstream); `test/features/hover_test.rb` pins both halves.
- `Node#set` on `<input type=file>` routes through `Browser#set_file_input_files` → `DOM.setFileInputFiles` (PR #2635), which sets `input.files` + fires `change`; multipart form submission then carries the bytes (PR #2654, `webapi/net/FormData.zig`). Both halves are guaranteed by the `MINIMUM_NIGHTLY_BUILD` floor — `attach_file` uploads end-to-end.
...
- `Page.addScriptToEvaluateOnNewDocument` works (PR #1993, merged 2026-03-30) — `Browser#create_page` registers the assembled `AutoScripts::JS` bundle (from `javascripts/`) once per session.
