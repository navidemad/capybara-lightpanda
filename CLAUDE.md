...
- `Node#set` on `<input type=file>` routes through `Browser#set_file_input_files` → `DOM.setFileInputFiles` (PR #2635), which sets `input.files` + fires `change`; multipart form submission then carries the bytes (PR #2654, `webapi/net/FormData.zig`). Both halves are guaranteed by the `MINIMUM_NIGHTLY_BUILD = 6672` floor — `attach_file` uploads end-to-end.
