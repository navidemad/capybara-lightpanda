...
- `javascripts/index.js` contains the XPath polyfill (`xpathFind` + `document.evaluate` shim) and Turbo tracking. `Browser#create_page` registers it via `Page.addScriptToEvaluateOnNewDocument` so Lightpanda auto-injects it on every navigation. No manual re-injection needed.
