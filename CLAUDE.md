...
- `javascripts/index.js` contains the XPath polyfill (`xpathFind` + `document.evaluate` shim) and Turbo tracking. On nightly builds (post-PR #1993), `Browser#create_page` registers it via `Page.addScriptToEvaluateOnNewDocument` for automatic injection on every navigation. On older builds where that method is a stub, `Driver` falls back to manual re-injection after each navigation. `Browser#verify_auto_scripts` detects which path to use after the first navigation.
