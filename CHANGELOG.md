...
> **Update Lightpanda before upgrading.** Requires a nightly build ≥ 8448 (the `<select>` option-value use-after-free fix, upstream PR #3087) **or a tagged release ≥ 0.3.7**, which is the first release containing that build. The driver refuses to start below either floor.

### Added

- **`raise_on_unhandled_modal` driver option** (Cuprite parity, their PR #320). A JS dialog that opens with no `accept_*`/`dismiss_*` wrapper in flight is resolved by Lightpanda's silent default — confirm → cancel, prompt → `null`, alert → dismissed — so a runaway `confirm` cancelled the action and the spec still passed. The driver now warns on stderr by default (`[capybara-lightpanda] A confirm dialog with text `…` opened, but the action was not wrapped in …`) and, with `raise_on_unhandled_modal: true`, raises `Capybara::Lightpanda::UnhandledModalError` from the click or `visit` that opened it. Wrapped dialogs are unaffected. Covered by `test/features/unhandled_modal_test.rb`.
- **`Node#exists?`** (Ferrum parity) — `true` while the node is still attached to a live document, `false` once it was removed or its document navigated away, without the `ObsoleteNode` round-trip every other operation makes.
...
- **One redirect no longer wedges `wait_for_network_idle` for the rest of the session.** Since Lightpanda build 8602 (upstream PR #3175, in 0.3.7 and every current nightly) every followed redirect hop emits its own `Network.requestWillBeSent` — same `requestId`, the 3xx attached as `redirectResponse`, and no `responseReceived` for the 3xx (Chrome's shape). The tracker pushed a second entry per hop and resolved the final response onto the *first*, so the hop's entry stayed pending forever: after any `redirect_to` (post-create, post-login), `page.driver.network.pending_connections` read 1 and every `wait_for_network_idle` / `Network#wait_for_idle!` burned its full timeout. Redirect responses now close the previous entry (as Ferrum does), responses resolve onto the newest open entry, and `network.traffic` shows the 302 on the hop that produced it and the 200 on the final URL. `status_code` still reports the final hop. Click/visit waits were never affected — they key on execution-context and Turbo signals, not the network counter.
...
- **Minimum Lightpanda build is now 8448** (was 8160), and the release floor moves with it to **0.3.7** (was 0.3.5, which is only build 8165 and predates all of the below). Six fixes justify it, each a wrong answer or a crash rather than a clean exception:
  ...
  - **PR #3058, build 8328 — `<option>`s inside `<optgroup>`.** Grouped options were invisible to `HTMLSelectElement` (`options`, `value`, `selectedIndex`, submission), so Capybara's `select` could not reach them at all and there was no gem-side workaround. This is the fix 0.3.6 (build 8318) just missed; 0.3.7 (build 8671) is the first release carrying it.
  - **PR #3080, build 8412 — timer cap.** One-shot `setTimeout` was capped at 512 per page (Airbnb-class pages schedule 600+); timers past the cap were silently dropped and page JS stalled in ways that read as Capybara timeouts. Now 2048.
  - **PR #3081/#3082, builds 8414/8419 — script `load`/`error` events, and `setAttribute("src")` scripts.** Inline scripts fired no `load`, a throwing script still fired `load`, and a dynamically inserted script whose `src` was set through `setAttribute` never loaded — the loader idioms (importmap shims, lazy widget bootstraps) that gate the UI a spec waits for.
  - **PR #3087, build 8448 — use-after-free on programmatic `<option>` value.** Exactly the path `select`/`Node#set` on a `<select>` drives; below it a spec could take the browser down mid-run (`DeadBrowserError`).
- **Capybara's `shadow_dom` shared specs now run** (9 examples, all green): `#shadow_root` finding/clicking/text inside nested shadow roots, and `node #path`, which now returns Selenium's `"(: Shadow DOM element - no XPath :)"` sentinel for an element under a shadow root instead of a `/SPAN[1]` computed from the shadow tree — a path XPath could never re-find. The feature was skipped for that one `#path` expectation while the rest already passed; upstream #3168/#3180/#3181 (build ≥8609) made the retest worthwhile.
- **`Element#drop` hands files to the browser via `DOM.setFileInputFiles`** instead of base64-ing them into one `Runtime.callFunctionOn` message (the transport Cuprite's #316 uses). A hidden `<input type=file>` is attached to the dropzone's document, pointed at the paths, and its `files` are moved into the `DataTransfer` before the drop fires; the input is removed afterwards. Lightpanda reads the bytes off disk itself, so the ~70 MB ceiling imposed by `--cdp-max-message-size` is gone and nothing is pinned in Ruby memory. Paths are resolved on the Lightpanda host, exactly like `attach_file`. `file.type` now comes from the browser's own sniffing rather than the gem's extension table.
