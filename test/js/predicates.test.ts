...
// --- isDraggable -------------------------------------------------------------
// Lightpanda doesn't implement the HTMLElement.draggable IDL property (probed
// 2026-08-24, nightly 8781), so Node#drag_to's scripts resolve draggability
// through this predicate. These tests pin the two halves: native-first (so the
// polyfill self-retires when upstream ships the getter — and keeps honoring
// SortableJS-style `el.draggable = true` property assignments), and the spec
// fallback that makes Capybara's `#drag_to HTML5` battery work today.

function maskDraggable(el: any) {
  // Simulate Lightpanda: no IDL getter — reading `.draggable` yields undefined.
  Object.defineProperty(el, "draggable", { value: undefined });
}

test("isDraggable: native property wins when the browser provides one", () => {
  const { predicates, document } = makeDom('<div id="t">x</div>');
  const el = document.getElementById("t") as any;
  Object.defineProperty(el, "draggable", { value: true });
  expect(predicates.isDraggable(el)).toBe(true);
});

test('isDraggable: draggable="true" attribute (Lightpanda fallback)', () => {
  const { predicates, document } = makeDom('<div id="t" draggable="true">x</div>');
  const el = document.getElementById("t");
  maskDraggable(el);
  expect(predicates.isDraggable(el)).toBe(true);
});

test('isDraggable: draggable="false" opts a default-draggable element out', () => {
  const { predicates, document } = makeDom('<img id="t" draggable="false" src="x.png">');
  const el = document.getElementById("t");
  maskDraggable(el);
  expect(predicates.isDraggable(el)).toBe(false);
});

test("isDraggable: <img> is draggable by default", () => {
  const { predicates, document } = makeDom('<img id="t" src="x.png">');
  const el = document.getElementById("t");
  maskDraggable(el);
  expect(predicates.isDraggable(el)).toBe(true);
});

test("isDraggable: <a href> is draggable by default, bare <a> is not", () => {
  const { predicates, document } = makeDom('<a id="l" href="/x">x</a><a id="b">y</a>');
  const link = document.getElementById("l");
  const bare = document.getElementById("b");
  maskDraggable(link);
  maskDraggable(bare);
  expect(predicates.isDraggable(link)).toBe(true);
  expect(predicates.isDraggable(bare)).toBe(false);
});

test("isDraggable: plain element without the attribute is not draggable", () => {
  const { predicates, document } = makeDom('<div id="t">x</div>');
  const el = document.getElementById("t");
  maskDraggable(el);
  expect(predicates.isDraggable(el)).toBe(false);
