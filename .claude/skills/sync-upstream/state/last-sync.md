...
- **htmldialog-polyfill-cleanup** [IN-FLIGHT] — branch `cleanup/drop-htmldialog-polyfill` (PR draft) drops the "Bug #4" block from `polyfills.js` and renames the matching describe in `test/features/upstream_bugs_test.rb` to "Bug #4 — HTMLDialogElement (upstream native)" so it stays as an upstream regression check. Validated locally 2026-05-13 against a `main`-derived binary (`1.0.0-dev.6192+e895ce81`, HEAD `e895ce81`): all 4 Bug #4 tests pass with the polyfill stripped. `Process::MINIMUM_NIGHTLY_BUILD` left at 6109 with a `TODO[release]` placeholder — must be bumped to the first nightly that contains PR #2435 (next nightly published after 2026-05-13 03:35 UTC) before the PR is marked ready and merged.
...
- **lightpanda-io/browser#2431** — "fix(cdp): remove duplicate Page.frameNavigated and fix context registration for iframes". State as of 2026-05-13: OPEN. The in-flight fix for issue #2400 (iframe contextId churn that drives our 13 `#switch_to_frame` / `#within_frame` failures on current nightly). When it merges, re-run the frame specs against the new nightly; if green, relax `Browser#find_in_frame`'s `refresh_frame_stack!` retry (kept as defense-in-depth, but the eager `refreshed = true` retry path should rarely fire).
