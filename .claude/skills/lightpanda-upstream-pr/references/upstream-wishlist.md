...
- **Upstream issue**: #2716, **Upstream PR**: #2717 (open as of 2026-06-12). Fix raises the ceiling to 100MB + overhead, matching Chrome's inbound `kReceiveBufferSizeForDevTools` (the WS reader buffer is lazily grown — 16KB at init — so well-behaved clients pay nothing; the oft-quoted 256MB is Chrome's *send* buffer). JSON-RPC-error alternative rejected in the issue: the command `id` lives inside the payload being refused. Branch `cdp-max-message-size`, worktree `/Users/navid/code/browser-cdp-message-size`, repro `repro/cdp-max-message-size/repro.sh` (exit 1 on nightly 6703, 0 with fix); new Zig test `Client: large websocket message` + updated 1009 assertion (full suite 792/792).
- **Gem workaround**: none viable — `execute_script` payloads are arbitrary user JS and can't be safely chunked. (A targeted mitigation like splitting known bundles into separate evaluates doesn't generalize.)
- **Drop-on-fix**: nothing to drop gem-side — purely unblocks large-script injection (decidim a11y specs et al.). Repro: serve any page, `session.execute_script("window.x = \"#{'x' * 600_000}\"")` → DeadBrowserError; same script at 500KB works. Upstream repro must be rebuilt CDP-only per the skill's rules (raw WS message > 524,442 bytes → observe close 1009).
...
- **Upstream issue/PR**: not yet filed. Fix is localized: an extra keyword arm next to the existing `@media` handling in `StyleManager.zig` (both paths) + a `layer` mapping in `atRuleTypeFor`. No layout dependency.
...
12b. **B18 — `@layer` blocks dropped from the cascade** — **cheap + high-confidence**: an extra keyword arm next to the existing `@media` handling in `StyleManager.zig` (flatten-the-wrapper v1, no layer-priority ordering needed) + a `layer` mapping in `atRuleTypeFor`. Verified with a 5-line CSS probe; unblocks Tailwind v4 apps (spree admin).
