...
### A49. `FormData.append(name, Blob|File)` stringifies the value — JS-built uploads send `[object Blob]`, never the bytes
...
### A51. `<form method="dialog">` navigates instead of closing its owner `<dialog>`
...
- **Drop-on-fix**: nothing gem-side to remove; unblocks the spree delete cluster and every Hotwire app using the `<dialog>` confirm pattern.
