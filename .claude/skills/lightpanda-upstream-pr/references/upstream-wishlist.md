...
11. **A44 — CDP WebSocket 512KiB inbound message cap** — **cheap + high-confidence**: the cap is a single constant (`Config.zig:37`) wired into one reader (`cdp/Connection.zig:45`); fix shape is grow-or-configure plus a JSON-RPC error instead of a hard close. Fully root-caused with a 2-line repro; kills every axe-core-style bundle injection (decidim a11y suite).
