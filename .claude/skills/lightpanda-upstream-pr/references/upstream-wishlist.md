...
- **Update 2026-07-24 — the two suspected mechanisms are now confirmed bugs in their own right (A49, A50 below).** Wire-level capture on nightly 8285 shows (a) `FormData.append(name, Blob|File)` stringifies the value, so a 200 KB file becomes a 119-byte body, and (b) the `.stream` arm really does send `Content-Length: 0` with no bytes. A50 is the stronger A45 candidate: an empty body under a `multipart/form-data` content type is precisely what raises `EOFError`/`EmptyContentError` in Rack. **Still not proven** that solidus/decidim reach that arm — the capture-proxy-in-front-of-the-real-app step above remains the deciding experiment. Note also that `Content-Length` always matched the bytes sent in every probe, so A45's original "truncated body" framing is wrong: the failures are content *loss*, not truncation.
...
- **Real-world impact**: an empty body paired with a `multipart/form-data` content type is *exactly* what makes Rack's multipart parser raise `EOFError` (rack 2.x) / `Rack::Multipart::EmptyContentError` (rack 3.x) in `handle_empty_content!`, which Rails turns into `ActionController::BadRequest`. That is the A45 signature seen on solidus and decidim. **Not yet proven** that those apps reach this arm — that still needs the capture-proxy-in-front-of-the-real-app step A45 prescribes — but it is the only known code path that emits a body-less request from a call site that asked for a body.
- **Want**: stream the body (chunked or buffered), or reject the request, rather than sending nothing.
- **Upstream issue**: not yet filed. Repro: `scratchpad/a45_wire.rb` (raw TCP capture comparing declared `Content-Length` against bytes received).
