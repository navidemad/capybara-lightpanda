...
- **Gem fix (possible but not shipped)**: a heuristic in `CLICK_JS` could, for a non-interactive container, redirect the dispatch to a "primary" descendant (e.g. deepest first-element-child) so the event bubbles up into the inner handler. **Risky**: `CLICK_JS` is the central click path for every test; the `<label><input></label>` double-toggle case and multi-child containers make a generic descend-heuristic error-prone. Worth it only with a full Capybara shared-battery (1401 ex.) validation pass — PR-worthy, not a quick change. Lower priority because solidus admin already needs the cuprite lane for C12.
- **Status**: gem-side, inherent (C2 family). **NOT an upstream candidate** — recorded so the select2 cluster isn't re-investigated as a Lightpanda or upstream bug. Dual-driver answer for solidus admin (cuprite), same as C12.
