...
Public nightly refreshed **2026-05-08 ~03:30 UTC** for all four platforms (Linux/macOS × x86_64/aarch64). Today's nightly main HEAD is `6e9156a8` "Merge pull request #2389 from lightpanda-io/interception-layer-on-serve" — 19 commits past yesterday's HEAD `f352c49a`. None require gem changes: PR #2378 (defensive assertion when reloading from about:blank — touches `Navigation.zig` only, does NOT close issue #2363, gem keeps `Target.disposeBrowserContext` workaround in `Browser#reset`), PR #2377 (move DOM version tracking from `Frame` to `Page` — perf refinement of #2365), PR #2382 (XHR teardown ordering — defense-in-depth alongside yesterday's #2374), PR #2383 (CSS.escape static binding fix), PR #2385 (avoid double-free on script error), PR #2388 (`Cache.clear` — foundation for a future `Network.clearBrowserCache` CDP method that does not yet exist; tracking only), PR #2389 (`InterceptionLayer` only on `.serve` mode — gem already uses serve mode, transparent), PR #2379 (V8 setter/static arity binding fix), PR #2380 (Illegal Constructor logging diagnostics), PR #2387/#2384 (CI / silence test warns). Local upstream worktree at `/Users/navid/code/browser` is fast-forwarded to `6e9156a8` (matches `origin/main` HEAD) on `main` branch, with untracked Claude tooling files only (no in-progress contribution). `Process::MINIMUM_NIGHTLY_BUILD = 6065` (set 2026-05-06 with the patchDispatch cleanup) still validates as the floor; today's commits are stability/perf/internal — no bump needed this round.
