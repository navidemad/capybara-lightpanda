...
Public nightly refreshed **2026-05-06 ~03:30 UTC** for all four platforms (Linux/macOS × x86_64/aarch64) at main HEAD `b573e44f` (PR #2368 merge — last commit before the build kicked off; `bf7cac2b` "Extract repeated listener switch/execution into helper" lands at 01:07 UTC and is also in this nightly). PR #2358 (FormData multipart) merged at 04:50 UTC, AFTER today's build cutoff — slated for tomorrow's nightly. macOS nightly continues to track the same 24h cycle as Linux. Local upstream worktree at `/Users/navid/code/browser` synced to `0c87fd67` (today's main HEAD, includes both PR #2368 and PR #2358) reports `1.0.0-dev.6041+0c87fd67` — note the `dev.NNNN` count diverges from upstream CI's `nightly.NNNN` because local clones lack the full ref set CI counts against. Today's published nightly is therefore in the 6052–6065 range; `Process::MINIMUM_NIGHTLY_BUILD = 6051` still passes the version-check until we choose to bump for the patchDispatch cleanup (see PR #2368 row below).
