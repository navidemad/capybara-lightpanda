...
Public nightly refreshed **2026-05-07 ~03:30 UTC** for all four platforms (Linux/macOS × x86_64/aarch64). Today's nightly main HEAD is `f352c49a` — 5 commits past yesterday's nightly HEAD `b573e44f`: PR #2371 (fast-path navigate, see below), PR #2374 (http_client teardown ordering, see below), `15021b73` "add WorkerLocation", `95f647d4` "Make SubtleCrypto work on Worker", `f352c49a` "Fix /json/version advertising ws://host:0/ when --port 0". macOS nightly continues to track the same 24h cycle as Linux. Local upstream worktree at `/Users/navid/code/browser` is at `7e0dc678` "Improve BodyInit" (2026-05-06) — 5 commits behind `origin/main`, on `main` branch with a small uncommitted `.gitignore` tweak (adds `.gitnexus`) plus untracked Claude tooling files; not fast-forwarded automatically by this skill. `Process::MINIMUM_NIGHTLY_BUILD = 6065` (set 2026-05-06 with the patchDispatch cleanup) still passes against today's nightly; no bump needed this round.
