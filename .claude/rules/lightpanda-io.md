...
Current floors enforced by the gem: `Process::MINIMUM_NIGHTLY_BUILD = 9058` **or** `Process::MINIMUM_RELEASE = 0.4.0` — the same commit on both channels (9058 = `git rev-list --count 0.4.0`). The 2026-09-06 bump 8875 → 9058 brought in #3298 Backspace/Delete editing (8937) and #3305's flag renames (8946); it stopped at 9058 because the release-channel lockstep caps the nightly floor at the newest tagged release — **the floor cannot pass 9058 until upstream tags something newer**, so #3375 (9071) and #3378 (9207) stay worked-around / build-gated. `lib/capybara/lightpanda/process.rb` carries the per-PR rationale of every bump; "Binary Distribution" below explains why two floors exist. There is no 0.3.8 — upstream went 0.3.7 → 0.4.0.
...
**WS handshake constraint since #3173 (build ≥8651, in 0.3.7)**: an upgrade request carrying any `Origin` header is 403'd, and `Host` must be an IP literal. `localhost` was rejected too until #3256 (≥8875), which allows the exact lowercase `localhost:<port>` form — bare `localhost`, `LOCALHOST:<port>` and `localhost.evil.com:<port>` still 403. The gem's spawn path is immune (it connects to the `address=` the server logs, and websocket-driver 0.8.1 sends no `Origin`); the 8875 floor clears the allowance, so a user-supplied `ws_url:` may now say `localhost:<port>` — pinned by `test/features/ws_url_host_test.rb`.
...
| **Emulation** | emulation.zig | `setUserAgentOverride` works (rejects `Mozilla`-containing UAs by design — go-rod workaround, #2704 closed as intended). `setDeviceMetricsOverride` drives `window.innerWidth`/`innerHeight`, `matchMedia`/`@media` evaluation, and `Page.getLayoutMetrics` — a JS-visible viewport, still no real layout. `mobile`/`scale` accepted-and-warned; `deviceScaleFactor` honored since #3231 (≥8815, 0 = don't override — feeds the screenshot renderer's scale). Since #3378 (≥9207) both override setters route through `Browser.setViewportOverride`, which re-evaluates every page's `MediaQueryList` and fires a real `change` (new `MediaQueryListEvent`) only where `matches` flipped — and `MediaQueryList.addListener`/`removeListener`, previously `.noop`, now map to `change` listeners. So `Driver#resize_window_to` (→ `Browser#set_viewport`) makes breakpoint-listening JS run mid-test; below 9207 it silently didn't. **JS side only** — the `@media` cascade of the loaded document still does not re-resolve (fixed at parse time; resize-then-`visit` remains the documented shape, see `Driver#resize_window_to`), verified 2026-09-06 on main 9213: after a resize `matchMedia` is true and `change` fired, `getComputedStyle(mobileCta).display` is still `none` until the next navigation. Params and semantics of the CDP call itself are unchanged. Pinned (both halves) by `viewport_test.rb` behind a build-gated `skip` (floor is release-capped at 9058). Also `setEmulatedMedia`, `setFocusEmulationEnabled`, `setTouchEmulationEnabled`, `setGeolocationOverride`/`clearGeolocationOverride` + `navigator.geolocation` (#3233, ≥8755) |
...
LP.configureLoading          (per-session loading toggles, params {subFrame, worker,
                              externalStylesheets, images}. Was "not applicable" while
                              iframes defaulted ON; #3305 (≥8946) flipped that, so
                              {subFrame: true} is now the gem's portable way to keep
                              switch_to_frame/within_frame working — limitation #6.)
...
2. **No rendering engine (CSS much improved)**
   ...
   - `getComputedStyle` resolves inline `style=` declarations, cascade-aware `display`/`visibility` (via StyleManager, so author stylesheets and `@layer` count), synthetic `width`/`height` matching `getBoundingClientRect`, and CSS **initial** values for `color`/`opacity`/`background-color`. Every other property returns `""` — there is still no cascade resolution for arbitrary properties. Inline keyword comparisons are case-insensitive since #3270 (≥8857). `checkVisibility` matches all active stylesheets
...
5. **External `<link rel="stylesheet">` fetch — ON by default in the gem** (build ≥6353)
   ...
   - That spelling is a fatal `UnknownOption` below 8946 (#3305), which the floor clears; the pre-#3305 `--enable-external-stylesheets` still parses upstream as a deprecated alias but the gem no longer sends it. (Per-session `LP.configureLoading {externalStylesheets: true}` also exists; the gem uses the CLI flag.)
...
6. **Iframes and workers are OFF by default from #3305 (build ≥8946) — LIVE, and handled by the gem**
   ...
   - In the floor since the 9058 bump, so every accepted binary behaves this way.
   ...
   - `--load-resources` is **one comma-separated value**, not a repeatable flag (`cli.zig` fills the `LoadResources` packed struct from a `,`-tokenized string; `full` sets every field). The gem sends `stylesheet` or `stylesheet,image`.
...
- `option.selected = true` did NOT deselect siblings in a single `<select>`, so the select carried two selected options and `select.value` reported the stale one. Fixed by #3375 (≥9071, **above the floor**). The gem works around it in `SELECT_OPTION_JS` (`node.rb`) by routing single-selects through `sel.value = this.value` instead of setting `this.selected`. That detour has its own defect the native path doesn't: when two `<option>`s share a `value`, `sel.value =` selects the *first* match, not the option Capybara resolved. Retire the detour (set `this.selected = true` unconditionally) only once `MINIMUM_NIGHTLY_BUILD` ≥ 9071.
...
- Landed through **9213** (main HEAD 2026-09-06; nightly serves 9204, release 0.4.0 = 9058). Everything **≤9058 is in the floor** — those fixes are assumed present and are described in the bullets they belong to (drag, screenshots, keyboard activation *and editing*, `<dialog>`, visibility keywords, WS `Host`, the `--load-resources` flag shape); don't re-list them here. In-floor one-liners that have no bullet of their own, kept so they are not re-derived: `Authorization` dropped on cross-origin redirect hops (#3267, 8880 — Network row); `WorkerNavigator` (#3288); synchronous XHR (#3240) and XHR `blob` responseType (#3284); immediate `<meta http-equiv=refresh>` navigation (#3283); runaway `MutationObserver` auto-disconnects after a non-draining streak (#3316); unknown CDP methods answer Chrome's `-32601` (#3324 — inert, the gem matches on message text never on codes); more `Runtime.*` passed through to the V8 inspector incl. `releaseObjectGroup`/`awaitPromise` (#3325); `DOM.getBoxModel` fills all four quads (#3323 — unused, geometry comes from JS `getBoundingClientRect`); `Page.setBypassCSP`/`bringToFront`/`Emulation.setLocaleOverride`/`setTimezoneOverride` accepted as no-ops (#3326); `Network.enable` enforces its buffer-size params (#3358 — the gem sends none, upstream defaults apply); `<body>` `clientWidth`/`clientHeight` = viewport (#3353 — scroll bullet); `target=_blank` opens a real popup `Frame` (#3327 — the gem is insulated, `CLICK_JS` assigns `location.href` itself and `Driver#switch_to_window` raises for any non-current handle); **BiDi** alongside CDP (#3209, 9058 — the gem stays on CDP).
  **Above the floor, gem-relevant, in build order**: **`option.selected = true` now deselects its siblings in a single `<select>`** (#3375, 9071 — retires the `sel.value =` detour in `SELECT_OPTION_JS`, see below); `<fieldset disabled>` gates descendant `HTMLElement.click()` and `willValidate` (#3376, 9074 — the gem's `_lightpanda.isDisabled` already matched `:disabled`, which walked fieldsets since #2315, so predicates were always right; what changes is that the `.click()` calls in `IMPLICIT_SUBMIT_JS` / checkbox toggling now correctly no-op inside a disabled fieldset); `contentEditable` IDL reflected (#3372, 9097 — `isContentEditable` still hardwired false, see limitation #4); `SameSite=Strict` site-for-cookies computed from the full ancestor chain, closing a 2-levels-deep bypass (#3293, 9102); `window.stop()` (#3368, 9110); runaway **IntersectionObserver** auto-disconnects like MutationObserver does (#3383, 9142 — a live-lock source on observer-heavy SPAs); **redirect handling restarts the whole HTTP pipeline** so redirect targets get robots/rate-limit/CORS checks (#3389, 9146 — touches only `src/network/HttpClient.zig`, the CDP emission is unchanged and upstream's `cdp.Network: redirect hop … carries redirectResponse` test pins it, so `Network#build_request_handler` stays valid); `performance.getEntriesByType("resource")` resource timing (#3391, 9154); **CORS behind `--experimental-features cors`** (#3002, 9196 — see the CORS bullet below); **`<select>` selectedness hardening** (#3402, 9202 — `option.selected = false` on a non-multiple select with display size <2 now runs `resetToDefaultSelection`, so exactly one option stays selected; `deselectOthers` gates on the `multiple` IDL rather than the content attribute; `option.label` returns an empty `label=""` verbatim; the non-standard `option.name` getter was removed. The gem is unaffected on all four: `UNSELECT_OPTION_JS` returns `'not_multiple'` before mutating anything on a single select, and nothing reads `option.label`/`option.name`); heap-limit and watchdog teardown SEGVs (#3392 9138 / #3406 9204 / #3407 9209 — `requestTerminate()` during V8 context disposal and a watchdog still armed through protocol teardown; both are crash-on-teardown paths the gem exercises on every `Driver#reset!` and at_exit, so they are stability wins, nothing to change); `matchMedia` `change` events on viewport override (#3378, 9207 — see the Emulation row; `viewport_test.rb` pins it behind a build-gated `skip`, since the floor is release-capped at 9058); WHATWG URL drive-letter conformance for non-`file:` schemes (#3217, 9213 — parser-level, not gem-visible).
- **Keyboard activation via CDP since #3264 (≥8842, in the floor since the 8875 bump)**: `Input.dispatchKeyEvent` builds a *trusted* `KeyboardEvent` (`initTrusted`) and routes it through `frame/user_input.zig`, which now (a) fires a `keypress` after a printable-or-Enter keydown without ctrl/meta — and skips the rest of the default action when a listener cancels it, so cancelling `keypress` correctly suppresses Enter's implicit submit; (b) synthesizes a trusted `PointerEvent` `click` for Enter on `<button>`, `<a href>` and `input[type=button|submit|reset|image]`, and for Space *keyup* on `<button>` and `input[type=button|submit|reset|image|checkbox|radio]`. That click is a real activation event (`PointerEvent.Proto = MouseEvent`, so `findClickActivationTarget` + `handleClick` run), so `send_keys(:enter)` now submits/navigates and `send_keys(:space)` now toggles a checkbox where both were previously no-ops. The gem cannot double-fire: `Keyboard#send_key_event` only ever sends `keyDown`/`keyUp` (plain characters go through `Input.insertText`), never CDP `type: "char"`, and `CLICK_JS`'s JS-synthesized events are untrusted so the new blocks skip them. **Never send `rawKeyDown`** — `input.zig` drops it as a Chrome-internal type (`if (params.type == .rawKeyDown) return;`), so a key dispatched that way never reaches the page at all. The gem did exactly that for every text-less key (Backspace, Delete, Tab, Escape, arrows) until 2026-09-06 (`Keyboard#raw_dispatch`); Enter/Space carry `text` and always went as `keyDown`, which is why activation worked while editing silently didn't. `Node#send_keys` gains the behavior for free; pinned by `test/features/keyboard_activation_test.rb`.
- **Keyboard editing via CDP since #3298 (≥8937, in the floor since the 9058 bump)**: a trusted Backspace/Delete keydown on an `<input>`/`<textarea>` runs `user_input.zig#editKey` → a cancelable trusted `beforeinput`, then the `text_entry.zig` mixin deletes one character on the caret's side (or the whole selection) and fires `input`, both with inputType `deleteContentBackward`/`deleteContentForward`; a listener cancelling `beforeinput` vetoes the edit. Pinned by `test/features/keyboard_editing_test.rb`. **Caret caveat, verified 2026-09-06 in `Input.zig#setValue`**: Lightpanda's `.value =` setter does NOT move the caret to the end (Chrome does; `_selection_start` stays wherever it was, 0 on a fresh control), so `node.set("abc")` followed by `send_keys(:backspace)` is a no-op *by upstream's caret arithmetic*, not a gem bug. A suite that types-then-corrects must place the caret first (`execute_script("arguments[0].setSelectionRange(n, n)", el)`). Two more gaps verified the same day (nightly 9204, main 9213), all three filed in the upstream wishlist (A54–A56): **`beforeinput.preventDefault()` does not veto the edit** — `user_input.zig#allowEdit` asks for a cancelable event but `InputEvent.initWithTrusted` hardcodes `_cancelable = false` for every InputEvent (right for `input`, wrong for `beforeinput`), so masked-input libraries that rely on the veto don't work; and **`<textarea>` selection setters reset to 0 until a value is assigned** — `text_entry.zig#select`/`setSelectionRange` read the assigned `_value`, which a textarea seeded only by its child text node lacks (an `<input value=…>` populates it from the attribute, so inputs are fine). `set` first, then select/correct.
- `<form method="dialog">` closes its nearest ancestor `<dialog>` (sets `returnValue` from the submitter's IDL `value`, fires `close`) and performs no navigation — #3053/PR #3054, build ≥8311, in the floor. Below it the submission fell through to a GET navigation and left the dialog open forever — the `<dialog>`+Turbo-confirm idiom Spree 5's admin uses for every destroy confirmation. There was never a gem-side workaround; the floor is the only defense, which is why it sits here. Separately, #3269 (≥8868) adds `dialog:not([open]) { display: none }` to the UA-stylesheet truth `checkVisibility` and `getComputedStyle().display` share, so a closed `<dialog>` **and its whole subtree** finally read as non-visible — below 8868 Capybara matched text and controls inside a never-opened dialog. In the floor since the 8875 bump; pinned by `test/features/upstream_bugs_test.rb` (Bug #4).
- `MutationObserver` available; `window.postMessage` across frames works
- CORS: **off unless opted in**. #3002 (≥9196) landed a real implementation gated behind `--experimental-features cors` (`Config.ExperimentalFeatures{ cors: false }` is the default, and upstream's README calls it experimental — #3410). The gem passes no such flag, so cross-origin fetches/XHR stay unenforced exactly as before. Only relevant if a suite opts in deliberately.
...
--load-resources <a,b,…>                   # ONE comma-separated value from
                                           # iframe|image|stylesheet|worker (or `full`); NOT
                                           # repeatable. ALL FOUR DEFAULT OFF since #3305 (≥8946).
...
--experimental-features cors               # Opt into experimental features (#3002, ≥9196).
                                           # `cors` is the only member today and defaults OFF;
                                           # the gem passes nothing, so CORS stays unenforced.
...
- **All four arch/OS pairs are published on every channel** (verified 2026-07-25
  ...
  re-narrow it.
- Latest release: **0.4.0 (2026-08-31) = build 9058**; 0.3.7 (2026-08-16) = 8671. There is no 0.3.8 — upstream jumped the minor. Tags drop the `v` prefix since 2026-04. Per release: `lightpanda-{aarch64,x86_64}-{linux,macos}` + `.deb` packages.
...
(`git rev-list --count <tag>`; 0.4.0 = 9058, 0.3.7 = 8671, 0.3.6 = 8318, 0.3.5 = 8165, 0.3.4 = 7708, 0.3.3 = 7562).
