...
Current floors enforced by the gem: `Process::MINIMUM_NIGHTLY_BUILD = 8875` (2026-08-26 bump: #3256 `localhost` in the WS `Host` 8875 + #3269 closed-`<dialog>` visibility 8868 + #3270 case-insensitive visibility keywords 8857 + #3264 keyboard activation 8842; subsumes the 8796 rationale — #3259 MouseEvent coordinate flooring, #3257 `draggable` IDL — and the 8448 one before it) **or** `Process::MINIMUM_RELEASE = 0.3.8`. `lib/capybara/lightpanda/process.rb` carries the per-PR rationale of every bump; "Binary Distribution" below explains why two floors exist.

**`0.3.8` will never be tagged** (verified 2026-09-04): upstream went 0.3.7 → **0.4.0 (2026-08-31) = build 9058**. The pin still *functions* — `0.4.0 >= 0.3.8`, so release-channel binaries are accepted and lockstep holds (9058 > 8875) — but it names a nonexistent version in the `BinaryError` message, and `test/unit/process_test.rb`'s lockstep table still carries the anticipated `0.3.8 => 8886` lower-bound row. Replace both with `0.4.0 => 9058` (now a verified `git rev-list --count 0.4.0`, not an estimate).
...
6. **Iframes and workers are OFF by default from #3305 (build ≥8946) — LIVE, and handled by the gem**
   ...
   - **Now shipping on both channels** (verified 2026-09-04): nightly serves 9151, release **0.4.0 = 9058**. This is no longer a future break.
   - **The gem's fix is in**: `Browser#configure_loading` sends `LP.configureLoading {subFrame: true, worker: true}` from `create_page` — per-BrowserContext, so it re-runs after every `Driver#reset!`. CDP, not CLI, deliberately: the `subFrame` param predates #3305 (no-op below 8946, load-bearing at/above), whereas the `--load-resources iframe` *value* only parses from 8946 and would force a floor bump. Pinned end-to-end by `test/features/frame_loading_test.rb`; green on nightly 9151.
   - The two flags the gem passes survive as **deprecated aliases** that still set their fields, so boot does not fail: `--enable-external-stylesheets` → `load_resources.stylesheet`, `--log-filter-scopes` → `--log-filter`. Both warn once per boot; `READY_PATTERN` is `/m` and unaffected.
...
- Landed through **9154** (main HEAD 2026-09-04; nightly serves 9151, release 0.4.0 = 9058). Everything **≤8875 is in the floor** — those fixes are assumed present and are described in the bullets they belong to (drag, screenshots, keyboard activation, `<dialog>`, visibility keywords, WS `Host`); don't re-list them here.
  **Above the floor, gem-relevant, in build order**: #3267 `Authorization` stripped on cross-origin redirects (8880, see the Network row); `WorkerNavigator` (#3288, 8892); synchronous XHR (#3240, 8912) and XHR `blob` responseType (#3284, 8925); **Backspace/Delete actually edit `<input>`/`<textarea>` via `Input.dispatchKeyEvent`** (#3298, 8937 — a `text_entry.zig` mixin firing `beforeinput`/`input` with `deleteContentBackward`/`deleteContentForward`; same family as #3264, so `Node#send_keys(:backspace)` gains real behavior and deserves a pin like `keyboard_activation_test.rb`); **#3305 iframes/workers off by default (8946) — limitation #6, handled**; immediate `<meta http-equiv=refresh>` navigation (#3283, 8977); runaway **MutationObserver** (#3316, 8982) and **IntersectionObserver** (#3383, 9142) now auto-disconnect after a non-draining streak — both were live-lock sources on observer-heavy SPAs; unknown CDP methods answer Chrome's `-32601` instead of the private `-31998` (#3324, 8984 — inert for the gem, which matches on message text, never on error codes); more `Runtime.*` passed through to the V8 inspector incl. `releaseObjectGroup`/`awaitPromise` (#3325, 9005); `DOM.getBoxModel` fills all four quads instead of zeroing padding/border/margin (#3323, 9002 — the gem reads geometry via JS `getBoundingClientRect`, not this); driver-setup no-ops accepted rather than erroring — `Page.setBypassCSP`, `Page.bringToFront`, `Emulation.setLocaleOverride`/`setTimezoneOverride` (#3326, 9011); `Network.enable` now enforces `maxTotalBufferSize`/`maxResourceBufferSize`/`maxPostDataSize` (#3358, 9037 — the gem sends `Network.enable` with no params, so upstream defaults apply); `<body>` `clientWidth`/`clientHeight` return the viewport (#3353, 9045 — see the scroll bullet); `target=_blank` anchors open a real popup `Frame` instead of being dropped as `not_implemented` (#3327, 9048 — the gem is insulated, `CLICK_JS` assigns `location.href` itself and `Driver#switch_to_window` raises for any non-current handle); **BiDi protocol** landed alongside CDP (#3209, 9058 — separate surface, the gem stays on CDP); **`option.selected = true` now deselects its siblings in a single `<select>`** (#3375, 9071 — retires the `sel.value =` detour in `SELECT_OPTION_JS`, see below); `<fieldset disabled>` gates descendant `HTMLElement.click()` and `willValidate` (#3376, 9074 — the gem's `_lightpanda.isDisabled` already matched `:disabled`, which walked fieldsets since #2315, so predicates were always right; what changes is that the `.click()` calls in `IMPLICIT_SUBMIT_JS` / checkbox toggling now correctly no-op inside a disabled fieldset); `contentEditable` IDL reflected (#3372, 9097 — `isContentEditable` still hardwired false, see limitation #4); `SameSite=Strict` site-for-cookies computed from the full ancestor chain, closing a 2-levels-deep bypass (#3293, 9102); `window.stop()` (#3368, 9110); **redirect handling restarts the whole HTTP pipeline** so redirect targets get robots/rate-limit/CORS checks (#3389, 9146 — touches only `src/network/HttpClient.zig`, the CDP emission is unchanged and upstream's `cdp.Network: redirect hop … carries redirectResponse` test pins it, so `Network#build_request_handler` stays valid); `performance.getEntriesByType("resource")` resource timing (#3391, 9154).
- **Keyboard activation via CDP since #3264 (≥8842, in the floor since the 8875 bump)**: `Input.dispatchKeyEvent` builds a *trusted* `KeyboardEvent` (`initTrusted`) and routes it through `frame/user_input.zig`, which now (a) fires a `keypress` after a printable-or-Enter keydown without ctrl/meta — and skips the rest of the default action when a listener cancels it, so cancelling `keypress` correctly suppresses Enter's implicit submit; (b) synthesizes a trusted `PointerEvent` `click` for Enter on `<button>`, `<a href>` and `input[type=button|submit|reset|image]`, and for Space *keyup* on `<button>` and `input[type=button|submit|reset|image|checkbox|radio]`. That click is a real activation event (`PointerEvent.Proto = MouseEvent`, so `findClickActivationTarget` + `handleClick` run), so `send_keys(:enter)` now submits/navigates and `send_keys(:space)` now toggles a checkbox where both were previously no-ops. The gem cannot double-fire: `Keyboard#send_key_event` only ever sends `keyDown`/`keyUp`/`rawKeyDown` (plain characters go through `Input.insertText`), never CDP `type: "char"`, and `CLICK_JS`'s JS-synthesized events are untrusted so the new blocks skip them. `Node#send_keys` gains the behavior for free; pinned by `test/features/keyboard_activation_test.rb`.
...
--load-resources iframe|image|stylesheet|worker   # Sub-resources to actually fetch,
                                           # repeatable. ALL FOUR DEFAULT OFF since #3305
                                           # (≥8946). `image` alone existed from #3230 (≥8834).
--enable-external-stylesheets              # DEPRECATED by #3305 (≥8946) in favour of
                                           # `--load-resources stylesheet`; still parses and still
                                           # sets the flag, now with a warning. Gem passes this.
...
--log-filter <SCOPE>                       # Renamed from --log-filter-scopes by #3305 (≥8946);
                                           ...
                                           # passes `--log-filter-scopes telemetry`.
