...
- **PR #2289** (by us, opened 2026-04-28, **APPROVED 2026-05-07**): **Page.getNavigationHistory + Page.navigateToHistoryEntry**. Maintainer's earlier change-requests addressed; latest review state is APPROVED, awaiting merge. When merged: `Browser#back` / `#forward` can switch from `history.back()` / `history.forward()` JS to native CDP commands, removing the JS workaround documented in CLAUDE.md.
- **PR #2305** (by us, opened 2026-04-28): **XPath 1.0 evaluator** (`Document.evaluate`, `XPathResult`/`XPathEvaluator`/`XPathExpression`, `DOM.performSearch` XPath routing). ~3,470 LOC Zig port of the gem polyfill; 91-case conformance battery passes. Maintainer left a COMMENTED review 2026-05-08 — review activity, not yet a decision. When merged: drop the entire `XPathEval` IIFE and `document.evaluate` polyfill from `index.js` (~700 LOC); also fixes XPath-in-iframes.
- **PR #2405** (by us, opened 2026-05-09): **Bound CDP macrotask drains so commands aren't queued behind page work**. Targets issue #2402 (CDP commands stall 15-20s on pages with sustained JS activity). Caps the per-tick macrotask drain so CDP processing isn't starved. When merged: gem specs hitting JS-heavy SPAs (Angular/RAF-loop pages) regain ~normal CDP latency.
...
Public nightly refreshed **2026-05-10 ~03:25 UTC** for all four platforms (Linux/macOS × x86_64/aarch64); today's nightly version is `1.0.0-nightly.6105+520d9688`, built from HEAD `520d9688` (PR #2398 worker-importscripts-segfault merge). Upstream `main` has moved further to `92d617d6` since the nightly cut — PR #2404 (our double-free fix) merged 2026-05-10 04:03 UTC, ~30 minutes after the nightly built, so it ships in tomorrow's. None of the new merges require gem changes: PR #2393 (`Add timeslice to scheduler` — perf refinement, internal), PR #2398 (`Defer page teardown while worker scripts are evaluating` — worker stability, by staylor; closes a worker `importScripts` segfault), PR #2404 (`Fix double-free in fetch when http_client.request fails synchronously`, by us — only triggers under `--obey-robots`, gem doesn't pass that flag, no gem cleanup). Local upstream worktree at `/Users/navid/code/browser` is fast-forwarded to `92607ad7` (Defer page teardown — one commit behind `main` HEAD `92d617d6`) on `main` branch, with untracked AGENTS.md/CLAUDE.md only (PR #2396 in flight, not merged). `Process::MINIMUM_NIGHTLY_BUILD = 6065` still validates as the floor; today's commits are stability/perf/internal — no bump needed this round.
...
| #2288 | CDP | `Page.getNavigationHistory` / `Page.navigateToHistoryEntry` not implemented. Our PR #2289 was APPROVED 2026-05-07 — awaiting merge. Once merged, gem can switch from `history.back()` / `history.forward()` JS to native CDP. |
...
| #2402 | CDP / latency | CDP commands stall 15-20 s on pages with sustained JS activity (`Runtime.evaluate('1+1')` takes ~15 s even though `fetch --dump html` returns the page in ~5 s). Filed 2026-05-09 by zaiddabaeen against build 6101. Our `Network#wait_for_idle` + Turbo activity tracker mitigate by waiting for the page to settle, but specs hitting Angular/RAF-loop SPAs would still see CDP latency. Our open PR #2405 ("Bound CDP macrotask drains so commands aren't queued behind page work") proposes a fix. |
