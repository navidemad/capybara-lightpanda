...
Current floors enforced by the gem: `Process::MINIMUM_NIGHTLY_BUILD = 8875` (2026-08-26 bump: #3256 `localhost` in the WS `Host` 8875 + #3269 closed-`<dialog>` visibility 8868 + #3270 case-insensitive visibility keywords 8857 + #3264 keyboard activation 8842; subsumes the 8796 rationale — #3259 MouseEvent coordinate flooring, #3257 `draggable` IDL — and the 8448 one before it) **or** `Process::MINIMUM_RELEASE = 0.3.8` — which upstream has NOT tagged yet (latest release 0.3.7 = 8671 is below the floor), so the release channel is nightly-only until 0.3.8 ships. `lib/capybara/lightpanda/process.rb` carries the per-PR rationale of every bump; "Binary Distribution" below explains why two floors exist.
...
6. **Iframes and workers are OFF by default from #3305 (build ≥8946) — INCOMING BREAK, not yet in a nightly**
   - `Config.LoadResources` went from `packed struct(u1)` (image only) to `packed struct(u4)` with `iframe`, `worker`, `stylesheet`, `image`, **all defaulting false**, and `Session.load_resources` lost its `= .{}` default so the config value is authoritative. `Frame.iframeAddedCallback` now returns early on `load_resources.iframe == false` (logging `warnDisabledIFrame`), so the parser registers the `<iframe>` in the DOM and nothing else happens.
   - **Gem impact: `Driver#switch_to_frame` / Capybara `within_frame` stop working.** The gem never passed a subframe flag because iframes were on by default; there is no deprecated alias that re-enables them (`--disable-subframes` was the opt-OUT and is now inert).
   - Not urgent *yet*: HEAD is 8977 but the rolling nightly still serves 8925 (verified 2026-08-28), so nothing is broken today. It breaks on the next nightly cut for everyone tracking nightly.
   - **The fix that works across the whole supported range is CDP, not CLI**: `LP.configureLoading {subFrame: true}` — the `subFrame` param predates #3305 and merely writes a different field after it (a no-op on <8946 where the default was already true, load-bearing on ≥8946). `--load-resources iframe` is CLI-equivalent but the *value* `iframe` only parses from 8946, so taking that route forces `MINIMUM_NIGHTLY_BUILD` ≥ 8946 in the same change.
   - The two flags the gem already passes survive as **deprecated aliases** that still set their fields, so boot does not fail: `--enable-external-stylesheets` → `load_resources.stylesheet` (`Config.zig:276` + the mapping at `:500`), and `--log-filter-scopes` → `--log-filter` (`:259-260`). Both now emit a deprecation warning per boot; `READY_PATTERN` is `/m` and unaffected.
   - Workers flipping off is the smaller half: it retires the "worker requests counted by `wait_for_idle`" watch item unless the gem opts back in.
...
- Landed 8332–8977 — everything ≤8875 is **in the floor** since the 2026-08-26 bump, including #3231 screenshots (8815), #3230 image loading (8834), #3264 keyboard activation (8842, own bullet below), #3270 case-insensitive visibility keywords (8857), #3269 closed-`<dialog>` visibility (8868, see the `<dialog>` bullet) and #3256 `localhost` in the WS `Host` (8875, see the CDP Server section). Above the floor, in build order: #3267 `Authorization` stripped on cross-origin redirects (8880, see the Network row); #3294 (8886, CI); `WorkerNavigator` (#3288, 8892); CDP-dispatched hover/mouse boundary events reworked in `user_input.zig` (#3287, 8890 — inert for the gem, whose `HOVER_JS` synthesizes `mouseover`+`mouseenter` in JS); synchronous XHR (#3240, 8912) and XHR `blob` responseType (#3284, 8925); **Backspace/Delete actually edit `<input>`/`<textarea>` when delivered via `Input.dispatchKeyEvent`** (#3298, 8937 — a new `text_entry.zig` mixin with `innerDelete`, selection tracking and `beforeinput`/`input` carrying `deleteContentBackward`/`deleteContentForward`; same family as #3264, so `Node#send_keys(:backspace)` gains real behavior and deserves the same kind of pin as `keyboard_activation_test.rb`); `searchParams` stays linked to its URL (#3299, 8939); **#3305 iframes and workers off by default (8946) — see limitation #6, the one item here that breaks the gem**; immediate `<meta http-equiv=refresh>` navigation (#3283, 8977). Everything below the floor line: `<iframe srcdoc>` (#3162, ≥8578); shadow DOM — `getHTML` (#3181), events retargeted (#3168) and nodes relocated (#3180) across shadow roots — the `shadow_dom` shared specs are un-gated since 2026-08-18 (9/9 green; `Node#path` returns Selenium's shadow-root sentinel); custom-element errors go through `window.reportError` (#3159), so `errors.js` lands them in `page_errors`; XPath `resolver` as function or object, `xlink:href` preserved, `getAttributeNS` for fixed-prefix namespaces (#3204, ≥8679); label activation no longer loops when the control sits inside the label (#3205, ≥8675); `Element.click()` honors the click-in-progress flag (#3210, ≥8688) — inert for the gem, whose `CLICK_JS` dispatches synthetic `MouseEvent`s; `el.style` re-parsed when the `style` attribute changes, so `setAttribute("style", …)` reaches `checkVisibility` (#3222, ≥8735); the bootstrap about:blank global is replaced on first navigation when a Runtime method other than enable touched it (#3218, ≥8741 — `create_page` only calls `Runtime.enable`, so the gem keeps the fast path; user `evaluate_script`-before-first-`visit` no longer leaks state into the first document); horizontal rect position computed from siblings + inline `translateX` (#3229, ≥8749); `device-width`/`device-height` media features resolve against the viewport instead of always-false (#3244, ≥8765); canvas `toDataURL`/`toBlob` return a 1×1 PNG instead of throwing (#3247, ≥8776); FileReader is properly async (#3220, ≥8732); multipart FormData follows the page's encoding, normalizes newlines, and fills `_charset_` inputs (#3249, ≥8781); `HTMLElement.click()` dispatches a `PointerEvent` (#3254, ≥8791 — its `Proto = MouseEvent`, so `instanceof MouseEvent` holds; inert for the gem, whose `CLICK_JS` synthesizes its own events); `draggable` IDL (#3257, ≥8793) and MouseEvent integer coordinate flooring (#3259, ≥8796) — see the drag bullet below; real screenshot rendering (#3231, ≥8815) — see limitation #2; opt-in image fetching via `--load-resources image` / `LP.configureLoading {images: true}` (#3230, ≥8834 — off by default; when enabled, image requests emit Network events and count toward `pending_connections`); inline-style keyword matching in `checkVisibility`/`getComputedStyle` is case-insensitive for `display`/`visibility`/`opacity`/`pointer-events` (#3270, ≥8857), so `style="display: NONE"` now hides where it used to read visible.
