...
**Last reviewed**: 2026-04-28 against Ferrum v0.17.2 (latest release 2026-03-24) and `main` HEAD `aa0b7adb` (2026-03-23). No new gem-relevant changes since prior review (one Unreleased commit on `main`: full-page screenshots no longer resize the window — Chrome-only, doesn't apply to Lightpanda).
...
- **[medium] `Browser::Network#wait_for_idle` / `wait_for_idle!` / `idle?` / `pending_connections`** — `lib/ferrum/network.rb:62-98` ↔ would live at `lib/capybara/lightpanda/network.rb` (currently absent). Ferrum tracks all in-flight HTTP via the `Network.requestWillBeSent` / `Network.responseReceived` / `Network.loadingFinished` / `Network.loadingFailed` events that Lightpanda already emits (`src/cdp/domains/network.zig:262-325`). Cuprite exposes it as `Driver#wait_for_network_idle` and Capybara users call it directly when an XHR isn't covered by Turbo/Hotwire. **Cross-reference for our Turbo activity tracker**: the gem's bespoke `_pendingTurboOps` counter in `index.js:17-70` only catches `turbo:*` events; a Ferrum-style network-idle tracker would catch every fetch (Turbo or otherwise) and could either replace the Turbo tracker (riskier — long-poll connections would pin `pending_connections > 0` indefinitely) or sit alongside it as a public `Driver#wait_for_network_idle` API. Lean toward "alongside, not replace" because the Turbo tracker terminates as soon as Turbo settles even when the underlying long-poll is still open.
...
**Last reviewed**: 2026-04-28 against Cuprite v0.17 (latest release 2025-05-11) and `main` HEAD `cc3a3da6` (2026-04-11). Active dev resumed Mar–Apr 2026 toward an unreleased version. No new commits since prior review — recent Cuprite features (rect, shadow_root, time inputs, focus-before-value, obscured?) are all already adopted.
...
- **`isVisible` is ~30 LOC instead of Cuprite's ~25** — Cuprite's `isVisible` (`lib/capybara/cuprite/javascripts/index.js:69-93`) just walks parents checking `display`/`visibility`/`opacity` from `getComputedStyle`. Our `_lightpanda.isVisible` (`lib/capybara/lightpanda/javascripts/index.js:834-865`) additionally rejects HEAD/SCRIPT/STYLE/NOSCRIPT/TEMPLATE/TITLE tags, walks ancestors looking for `[hidden]` and closed `<details>`, and special-cases `<summary>` inside a closed `<details>`. The extra logic exists because Lightpanda's UA stylesheet is incomplete (see `upstream-wishlist.md` A24) — Chrome's UA stylesheet hides those tags + closed-`<details>` children + `[type=hidden]` inputs natively, so Cuprite gets it for free via `getComputedStyle().display === 'none'`. When A24 lands upstream, we can collapse `isVisible` to roughly Cuprite's shape (~20 LOC saved).
