...
**Last reviewed**: 2026-04-29 against Ferrum v0.17.2 (latest release 2026-03-24) and `main` HEAD `25367b52` (2026-04-22). No new commits on `main` since prior review (the `captureBeyondViewport` Unreleased entry is Chrome-only). No gem-relevant changes — Outstanding adoption candidates and Diverged-on-purpose entries below stay valid.
...
- **[tiny] `Utils::Attempt.with_retry(errors:, max:, wait:)`** — `lib/ferrum/utils/attempt.rb` ↔ would live at `lib/capybara/lightpanda/utils/attempt.rb` (currently we only have `utils/event.rb`). Our `Browser#with_default_context_wait` does a *single* event-driven retry on `NoExecutionContextError` — it waits for a default-context signal then yields again. Extracting a generic loop (`max:` attempts, `wait:` sleep, configurable `errors:`) matches Ferrum's surface (`INTERMITTENT_ATTEMPTS = 6`, `INTERMITTENT_SLEEP = 0.1`) and lets us reuse it for other transient `BrowserError` cases.
...
**Last reviewed**: 2026-04-29 against Cuprite v0.17 (latest release 2025-05-11) and `main` HEAD `cc3a3da6` (2026-04-11). No new commits since prior review — recent Cuprite features (rect, shadow_root, time inputs, focus-before-value, obscured?) are all already adopted.
...
- **`isVisible` is ~30 LOC instead of Cuprite's ~25** — Cuprite's `isVisible` (`lib/capybara/cuprite/javascripts/index.js:69-93`) just walks parents checking `display`/`visibility`/`opacity` from `getComputedStyle`. Our `_lightpanda.isVisible` (`lib/capybara/lightpanda/javascripts/index.js:834-865`) additionally rejects HEAD/SCRIPT/STYLE/NOSCRIPT/TEMPLATE/TITLE tags, walks ancestors looking for `[hidden]` and closed `<details>`, and special-cases `<summary>` inside a closed `<details>`. The extra logic exists because Lightpanda's UA stylesheet was incomplete — Chrome's UA stylesheet hides those tags + closed-`<details>` children + `[type=hidden]` inputs natively, so Cuprite gets it for free via `getComputedStyle().display === 'none'`. **PR #2294 (by us) MERGED 2026-04-29**, awaiting next public nightly (~04-30 03:33 UTC, ~build 5918+). Once `MINIMUM_NIGHTLY_BUILD` is bumped, collapse `isVisible` to roughly Cuprite's shape (~20 LOC saved).
