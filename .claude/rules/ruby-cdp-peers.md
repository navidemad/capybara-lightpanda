...
**Last reviewed**: 2026-04-28 against Ferrum v0.17.2 (latest release 2026-03-24) and `main` HEAD `25367b52` (2026-04-22). One Unreleased commit on `main` since prior review: full-page screenshots use `captureBeyondViewport` instead of resizing the window (#580). Chrome-only, doesn't apply to Lightpanda — no gem-relevant changes.
...
**Last reviewed**: 2026-04-28 against Cuprite v0.17 (latest release 2025-05-11) and `main` HEAD `cc3a3da6` (2026-04-11). No new commits since prior review — recent Cuprite features (rect, shadow_root, time inputs, focus-before-value, obscured?) are all already adopted.
...
- **`isVisible` is ~30 LOC instead of Cuprite's ~25** — Cuprite's `isVisible` (`lib/capybara/cuprite/javascripts/index.js:69-93`) just walks parents checking `display`/`visibility`/`opacity` from `getComputedStyle`. Our `_lightpanda.isVisible` (`lib/capybara/lightpanda/javascripts/index.js:834-865`) additionally rejects HEAD/SCRIPT/STYLE/NOSCRIPT/TEMPLATE/TITLE tags, walks ancestors looking for `[hidden]` and closed `<details>`, and special-cases `<summary>` inside a closed `<details>`. The extra logic exists because Lightpanda's UA stylesheet is incomplete (wishlist A24) — Chrome's UA stylesheet hides those tags + closed-`<details>` children + `[type=hidden]` inputs natively, so Cuprite gets it for free via `getComputedStyle().display === 'none'`. **PR #2294 (by us, OPEN 2026-04-28)** addresses this upstream; when merged, collapse `isVisible` to roughly Cuprite's shape (~20 LOC saved).
